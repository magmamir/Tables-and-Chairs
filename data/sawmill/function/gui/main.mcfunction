...
data modify storage sawmill:temp savedData set from entity @s item.components."minecraft:custom_data".tac
## V4.0
data modify storage sawmill:temp savedData set from entity @s ArmorItems[3].components."minecraft:custom_data".tac
...
execute store result score #locked_slot_count tac.temp if data storage sawmill:temp inventory[{count:1, components:{"minecraft:custom_data":{lockedSlot:1b}}}]
...
execute if score #save_data tac.temp matches 1 run data modify entity @s item.components."minecraft:custom_data".tac set from storage sawmill:temp savedData
## V4.0
execute if score #save_data tac.temp matches 1 run data modify entity @s ArmorItems[3].components."minecraft:custom_data".tac set from storage sawmill:temp savedData


## CHUCKCHUK SECTION: >>>

#Slot 18 (Slot for The GUI Element Visuals)
execute unless data block ~ ~ ~ Items[{Slot:18b,id:"minecraft:poppy",components:{"minecraft:custom_data":{Tags:["chk.gui"]}}}] run function sawmill:gui/slot_return/slot_18
...
#data modify block ~ ~ ~ Items append value {id:poppy,count:1,Slot:18b,tag:{CustomModelData:9001501,Tags:["chk.gui","chk.inventory_remove"]}}

#Clear any Unwanted player items
clear @a[distance=0..6.5] minecraft:poppy[custom_data~{Tags:["chk.gui"]}]
