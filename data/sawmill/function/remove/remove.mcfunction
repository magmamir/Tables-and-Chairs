kill @e[distance=0..4,nbt={Item:{components:{"minecraft:custom_data":{Tags:["chk.gui"]}}}}]
...
kill @e[type=item,nbt={Item:{components:{"minecraft:custom_data":{lockedSlot:1b}}}}]

kill @s


## If Destroying through other means
# execute if block ~ ~ ~ barrel run summon item ~ ~ ~ {Tags:["chk.temp"],Item:{id:"minecraft:sand",count:1}}
