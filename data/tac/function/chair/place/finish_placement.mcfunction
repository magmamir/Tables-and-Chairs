...
execute store result score Current_Chair chk.style run data get entity @s item.components."minecraft:custom_model_data"
scoreboard players operation Current_Chair chk.style += @p[distance=0..6,scores={chk.style=0..}] chk.type
execute store result entity @s item.components."minecraft:custom_model_data" int 1 run scoreboard players operation Current_Chair chk.style += @p[distance=0..6,scores={chk.style=0..}] chk.style

# # Modify the Data to be the correct Style
# execute as @s store result score Current_Chair chk.style run data get entity @s item.components."minecraft:custom_model_data"
# execute as @s store result entity @s item.components."minecraft:custom_model_data" int 1 run scoreboard players operation Current_Chair chk.style += @p[distance=0..6,scores={chk.style=0..}] chk.style
