...
execute at @s store result entity @e[distance=0..0.4,tag=chk.table.top,limit=1] item.components."minecraft:custom_model_data" int 1 run scoreboard players get @s chk.start_model
...
execute store result entity @s item.components."minecraft:custom_model_data" int 1 run scoreboard players operation Temp_Chair chk.style += @s chk.start_model
