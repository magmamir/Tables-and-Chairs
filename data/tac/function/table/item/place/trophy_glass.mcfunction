...
execute as @e[distance=0..0.5,type=item_frame,tag=chk.table.item.trophy.case] run data modify entity @s Item set value {id:"minecraft:orange_stained_glass",count:1,components:{"minecraft:custom_model_data":9007000,"minecraft:custom_data":{Tags:[chk.item.temporary]}}}


#Return Old Glass
execute as @e[distance=0..0.5,tag=chk.table.item.trophy.case] at @s run function tac:table/item/place/return_trophy_glass

# execute as @e[distance=0..0.5,tag=chk.table.item.trophy.case] if score @s chk.case_color matches 0 run summon item ~ ~ ~ {Item:{id:"minecraft:glass",count:1}}
# execute as @e[distance=0..0.5,tag=chk.table.item.trophy.case] if score @s chk.case_color matches 1 run summon item ~ ~ ~ {Item:{id:"minecraft:white_stained_glass",count:1}}
...
execute as @e[distance=0..0.5,tag=chk.table.item.trophy.case] store result score TrophyCase chk.case_size run data get entity @s Item.components."minecraft:custom_model_data"
...
#1 Block
execute as @s[tag=chk.table.item.trophy.silverfish] as @e[distance=0..0.5,tag=chk.table.item.trophy.case] store result entity @s Item.components."minecraft:custom_model_data" int 1 run scoreboard players add TrophyCase chk.case_size 100
#2 Block
execute as @s[tag=chk.table.item.trophy.creeper] as @e[distance=0..0.5,tag=chk.table.item.trophy.case] store result entity @s Item.components."minecraft:custom_model_data" int 1 run scoreboard players add TrophyCase chk.case_size 200
#3 Block
execute as @s[tag=chk.table.item.trophy.enderman] as @e[distance=0..0.5,tag=chk.table.item.trophy.case] store result entity @s Item.components."minecraft:custom_model_data" int 1 run scoreboard players add TrophyCase chk.case_size 300
